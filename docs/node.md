...
- Vercel only supports defining major node versions (e.g. 24.x). Minor and patch versions are managed by Vercel and can not be defined.
...
- We define the node version in the package.json as well (e.g. engines -> node -> 24.x), which will be respected by vercel.
