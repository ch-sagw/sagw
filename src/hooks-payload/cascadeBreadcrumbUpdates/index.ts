...
import {
  resumeRedirectCreationForOperation,
  suppressRedirectCreationForOperation,
...
const updateChildBreadcrumbs = async (
  ...
  const childPages = await findChildPages(payload, req, parentDocumentId, tenantId);
...
export const hookCascadeBreadcrumbUpdates: CollectionAfterChangeHook = async ({
  ...
  try {
    ...
    if ((slugChanged || navigationTitleChanged) && collectionSlug) {
      ...
      try {
        suppressRedirectCreationForOperation(collectionSlug, String(docId));
        ...
        resumeRedirectCreationForOperation(collectionSlug, String(docId));
      }
    }
