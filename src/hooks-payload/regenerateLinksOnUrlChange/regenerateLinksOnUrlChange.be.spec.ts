...
test('Regenerates source links if target url changes (sagw tenant)', {
  tag: '@linking',
}, async () => {
  let result: any;
  ...
  try {
    ...
    await payload.update({
      ...
      data: {
        slug: `Overview Page changed ${time}`,
      },
      id: level1.id,
    });
  ...
  await expect(linkElement.de)
    .toStrictEqual(`/de/overview-page-changed-${time}`);
...
test('Regenerates source links if target url changes (non-sagw tenant)', {
  ...
  await expect(linkElement.de)
    .toStrictEqual(`/de/${time}-tenant-1/overview-page-changed-${time}`);
